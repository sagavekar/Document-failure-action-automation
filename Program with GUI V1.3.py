...
def Inbound_auto(assignee_key, file_path):
    ...
    for index, row in enumerate(Inbound_failures.used_range.rows[7:]):
        ...
        if (
            ...
            row[8].value = "As per Resolution Column - Email sent to supplier to check" # Action Performed column
...
def Outbound_auto(assignee_key, file_path):
    ...
    def email(P, target_wb_path, td_in_draft):
        ...
        Kindly check and confirm if you have recived POs already. 

        Please check and confirm the status or amendments.<br><br>
