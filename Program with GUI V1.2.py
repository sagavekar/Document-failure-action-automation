...
def Outbound_auto(assignee_key, file_path):
    ...
    for index, row in enumerate(Outbond_failures.used_range.rows[6:]):
        ...
        if (
            ...
            and (search("check with supplier", str(row[4].value), IGNORECASE) or  search("if they did not receive the PO", str(row[4].value), IGNORECASE) or search("but no response was received", str(row[4].value), IGNORECASE))
